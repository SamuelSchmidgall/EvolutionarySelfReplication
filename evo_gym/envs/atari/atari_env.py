...
class AtariEnv(evo_gym.Env, utils.EzPickle):
    ...
    def __init__(
        ...
        if self._obs_type == 'ram':
            ...
            if self.game == "pong":
                self.observation_space = spaces.Box(low=0, high=1, dtype=np.uint8, shape=(128*3,))
    ...
    def step(self, a):
        ...
        if self.game == "boxing" or self.game == "space_invaders":
            num_steps = 1
        elif self.game == "breakout":
            num_steps = 1
        elif isinstance(self.frameskip, int):
        ...
        return ob, self.ale.game_over(), {"ale.lives": self.ale.lives(), "point": reward}
    ...
    def _get_obs(self):
        if self._obs_type == 'ram':
            ...
            if self.game == "pong":
                ...
                state = np.zeros(128*3)
                state[player_paddle_y//2] = 1
                state[ball_x//2 + 128] = 1
                state[ball_y//2 + 256] = 1
                return state
            return ram
        elif self._obs_type == 'image':
            img = self._get_image()
    ...
    def render(self, mode='human'):
        ...
        if mode == 'rgb_array':
            ...
            from evo_gym.envs.classic_control import rendering
