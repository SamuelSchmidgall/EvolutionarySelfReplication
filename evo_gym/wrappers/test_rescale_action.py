...
import evo_gym
from evo_gym.wrappers import RescaleAction


def test_rescale_action():
    env = evo_gym.make('CartPole-v1')
    with pytest.raises(AssertionError):
        env = RescaleAction(env, -1, 1)
    del env

    env = evo_gym.make('Pendulum-v0')
    wrapped_env = RescaleAction(evo_gym.make('Pendulum-v0'), -1, 1)
