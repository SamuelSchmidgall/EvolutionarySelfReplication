...
import evo_gym
from evo_gym.wrappers import TransformReward
...
def test_transform_reward(env_id):
    ...
    for scale in scales:
        ...
        wrapped_env = TransformReward(evo_gym.make(env_id), lambda r: scale * r)
    ...
    max_r = 0.0002
    env = evo_gym.make(env_id)
    wrapped_env = TransformReward(evo_gym.make(env_id), lambda r: np.clip(r, min_r, max_r))
    action = env.action_space.sample()
    ...
    # use case #3: sign
    env = evo_gym.make(env_id)
    wrapped_env = TransformReward(evo_gym.make(env_id), lambda r: np.sign(r))
